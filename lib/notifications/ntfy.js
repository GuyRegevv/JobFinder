 ...
 */
export function formatJobsMessage(jobs, maxJobs = 5) {
  if (!jobs || jobs.length === 0) {
    return "No new jobs found.";
  }

  const displayJobs = jobs.slice(0, maxJobs);
  const lines = displayJobs.map((job) => {
...
export async function notifyNewJobs(jobs, options = {}) {
  const { topic, server, clickUrl } = options;
  ...
  const title = `${jobs.length} New Job${jobs.length > 1 ? "s" : ""} Found`;
